...
const blogPosts = [
  ...
  },
  {
    ...
    gradient: 'from-noir-soft via-mauve-dusty/20 to-noir',
    letter: 'A',
  },
  {
  ...
  return (
    <SiteLayout>
      ...
      <section className="relative pt-32 pb-16 md:pt-40 md:pb-20 overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-br from-champagne/40 via-blush/30 to-sandalwood/20" />
        ...
        <div className="relative max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <motion.h1
            ...
            className="font-cormorant text-4xl sm:text-5xl md:text-6xl text-noir leading-[1.1] mb-8"
          >
            The Journal
          </motion.h1>
          <motion.div
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <FadeInSection>
            <Link href={`/blog/${featuredPost.slug}`} className="group block">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-0 border border-zari-gold/10 hover:border-zari-gold/20 transition-all duration-500">
                <div className={`aspect-[16/9] lg:aspect-auto bg-gradient-to-br ${featuredPost.gradient} overflow-hidden`}>
                  <div className="w-full h-full flex items-center justify-center group-hover:scale-105 transition-transform duration-700">
                    <span className="font-cormorant text-noir/10 text-9xl italic">{featuredPost.letter}</span>
                  </div>
                </div>
                <div className="p-8 md:p-12 flex flex-col justify-center">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <span className="font-dm-sans text-[10px] text-noir/30">{featuredPost.readTime}</span>
                  </div>
                  <h2 className="font-cormorant text-2xl md:text-3xl lg:text-4xl text-noir mb-4 group-hover:text-zari-gold transition-colors duration-300">
                    {featuredPost.title}
                  </h2>
                  <p className="font-dm-sans text-sm text-noir/50 leading-relaxed mb-6">{featuredPost.excerpt}</p>
                  <div className="flex items-center justify-between">
                    <span className="font-dm-sans text-xs text-noir/30">{featuredPost.date}</span>
          ...
          <div className="flex gap-12">
            ...
            <div className="flex-1">
              ...
              <div className="mb-8">
                <div className="relative">
                  <svg className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-noir/20" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
                  <input
                    type="text"
                    value={searchQuery}
                    onChange={(e) => setSearchQuery(e.target.value)}
                    placeholder="Search articles..."
                    className="w-full bg-transparent border border-zari-gold/20 pl-12 pr-4 py-3 font-dm-sans text-sm text-noir placeholder:text-noir/20 focus:border-zari-gold focus:outline-none transition-colors duration-300"
                  />
                </div>
              </div>
                ...
                {blogCategories.map((cat) => (
                  <button
                    ...
                    className={`px-3 py-1.5 font-dm-sans text-[10px] tracking-wider uppercase whitespace-nowrap transition-all duration-300 ${
                      activeCategory === cat
                        ? 'bg-noir text-ivory'
                        : 'text-noir/40 hover:text-noir hover:bg-blush/30 border border-zari-gold/10'
                    }`}
                  >
                    {cat}
                  </button>
                  ...
                  <FadeInSection key={post.slug} delay={index * 0.1}>
                    <Link href={`/blog/${post.slug}`} className="group block">
                      <div className={`aspect-[16/10] bg-gradient-to-br ${post.gradient} rounded-sm overflow-hidden mb-4`}>
                        <div className="w-full h-full flex items-center justify-center group-hover:scale-105 transition-transform duration-700">
                          <span className="font-cormorant text-noir/10 text-7xl italic">{post.letter}</span>
                        </div>
                      </div>
                      <div className="flex items-center gap-3 mb-2">
                        <span className="font-dm-sans text-[10px] tracking-wider uppercase text-zari-gold">{post.category}</span>
                        <span className="w-1 h-1 bg-noir/10 rounded-full" />
                        <span className="font-dm-sans text-[10px] text-noir/30">{post.readTime}</span>
                      </div>
                      <h3 className="font-playfair text-lg text-noir mb-2 group-hover:text-zari-gold transition-colors duration-300 leading-snug">{post.title}</h3>
                      <p className="font-dm-sans text-xs text-noir/50 leading-relaxed mb-3 line-clamp-3">{post.excerpt}</p>
                      <span className="font-dm-sans text-[10px] text-noir/30">{post.date}</span>
                    </Link>
                  </FadeInSection>
                ))}
              </div>
              ...
              {filteredPosts.length === 0 && (
                <div className="text-center py-16">
                  <p className="font-cormorant text-2xl text-noir/30 italic">No articles found</p>
                  <p className="font-dm-sans text-sm text-noir/20 mt-2">Try a different search or category</p>
                </div>
              )}
            </div>
            ...
            <aside className="hidden lg:block w-72 shrink-0">
              <div className="sticky top-32 space-y-8">
                {/* Categories */}
                <div className="p-6 border border-zari-gold/10">
                  <p className="font-dm-sans text-xs tracking-wider uppercase text-noir/40 mb-4">Categories</p>
                ...
                {/* WhatsApp CTA */}
                <div className="p-6 bg-[#25D366]/5 border border-[#25D366]/20">
                  <p className="font-playfair text-base text-noir mb-2">Quick Question?</p>
                  <p className="font-dm-sans text-xs text-noir/40 mb-4 leading-relaxed">
      ...
      <section className="py-16 md:py-24 bg-blush/30">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <FadeInSection>
            ...
            <h2 className="font-cormorant text-3xl md:text-4xl text-noir mb-4">Subscribe to The Journal</h2>
            ...
            <form onSubmit={(e) => e.preventDefault()} className="flex max-w-md mx-auto">
              <input
                ...
                className="flex-1 bg-ivory border border-zari-gold/20 px-4 py-3 font-dm-sans text-sm text-noir placeholder:text-noir/20 focus:border-zari-gold focus:outline-none transition-colors duration-300"
              />
              <button
                type="submit"
                className="px-6 py-3 bg-noir text-ivory font-dm-sans text-xs tracking-wider uppercase hover:bg-noir-soft transition-colors duration-300"
              >
                Subscribe
              </button>
            </form>
