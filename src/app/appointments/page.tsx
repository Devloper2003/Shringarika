...
export default function AppointmentsPage() {
  ...
  return (
    <SiteLayout>
      ...
      {/* Trust Indicators */}
      <section className="py-8 border-y border-zari-gold/10 bg-[#0d1220]">
          ...
          <div className="flex items-center justify-center gap-8 md:gap-16">
            {[
              ...
              <div key={stat.label} className="text-center">
                <span className="font-cormorant text-2xl md:text-3xl text-zari-gold block drop-shadow-[0_0_8px_rgba(212,175,55,0.2)]">{stat.number}</span>
                <span className="font-dm-sans text-[10px] tracking-wider uppercase text-ivory/40">{stat.label}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <FadeInSection>
            <div className="text-center mb-16">
              ...
              <h2 className="font-cormorant text-3xl md:text-4xl lg:text-5xl text-ivory mb-6">Consultation Types</h2>
            </div>
          </FadeInSection>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {consultationTypes.map((type, index) => (
              <FadeInSection key={type.id} delay={index * 0.15}>
                <div className="gaming-card p-8 hover:border-zari-gold/30 transition-all duration-500 group h-full flex flex-col">
                  <span className="text-3xl text-zari-gold/60 group-hover:text-zari-gold group-hover:drop-shadow-[0_0_8px_rgba(212,175,55,0.4)] transition-all duration-500 mb-4 block">{type.icon}</span>
                  <h3 className="font-playfair text-xl text-ivory mb-2">{type.title}</h3>
                  <div className="flex items-center gap-3 mb-4">
                    <span className="font-dm-sans text-xs text-ivory/40">{type.duration}</span>
                    <span className="w-1 h-1 bg-zari-gold/40 rounded-full" />
                    <span className="font-dm-sans text-xs text-zari-gold">{type.price}</span>
                  </div>
                  <p className="font-dm-sans text-sm text-ivory/50 leading-relaxed mb-6 flex-1">{type.description}</p>
                  <div className="pt-4 border-t border-zari-gold/10">
                    <p className="font-dm-sans text-[10px] tracking-wider uppercase text-ivory/30 mb-3">Includes</p>
                    <ul className="space-y-1.5">
                      {type.includes.map((item) => (
                        <li key={item} className="font-dm-sans text-xs text-ivory/40 flex items-start gap-2">
                  ...
                  <button
                    ...
                    className="mt-6 w-full py-3 border border-zari-gold/40 text-zari-gold font-dm-sans text-xs tracking-wider uppercase hover:bg-zari-gold hover:text-noir hover:shadow-[0_0_20px_rgba(212,175,55,0.3)] transition-all duration-500"
                  >
                    Select & Book
                  </button>
                </div>
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <FadeInSection>
            <div className="text-center mb-12">
              ...
              <h2 className="font-cormorant text-3xl md:text-4xl text-ivory mb-6">Book an Appointment</h2>
          ...
          <FadeInSection delay={0.2}>
            <form onSubmit={(e) => e.preventDefault()} className="gaming-card p-8 md:p-12">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                  <label className="block font-dm-sans text-xs tracking-wider uppercase text-ivory/40 mb-2">Consultation Type *</label>
                  <select
                    value={selectedType}
                    onChange={(e) => setSelectedType(e.target.value)}
                    className="w-full bg-noir-soft/50 border border-zari-gold/20 px-4 py-3 font-dm-sans text-sm text-ivory/50 focus:border-zari-gold focus:shadow-[0_0_12px_rgba(212,175,55,0.15)] focus:outline-none transition-all duration-300"
                  >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                  <label className="block font-dm-sans text-xs tracking-wider uppercase text-ivory/40 mb-2">Preferred Date *</label>
                  <input
                    type="date"
                    className="w-full bg-noir-soft/50 border border-zari-gold/20 px-4 py-3 font-dm-sans text-sm text-ivory/50 focus:border-zari-gold focus:shadow-[0_0_12px_rgba(212,175,55,0.15)] focus:outline-none transition-all duration-300"
                  />
                ...
                <div>
                  <label className="block font-dm-sans text-xs tracking-wider uppercase text-ivory/40 mb-2">Preferred Time *</label>
                  <select className="w-full bg-noir-soft/50 border border-zari-gold/20 px-4 py-3 font-dm-sans text-sm text-ivory/50 focus:border-zari-gold focus:shadow-[0_0_12px_rgba(212,175,55,0.15)] focus:outline-none transition-all duration-300">
              ...
              <div className="mb-8">
                <label className="block font-dm-sans text-xs tracking-wider uppercase text-ivory/40 mb-2">Message (Optional)</label>
                <textarea
                  rows={4}
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <FadeInSection>
            <div className="text-center mb-16">
              ...
              <h2 className="font-cormorant text-3xl md:text-4xl lg:text-5xl text-ivory mb-6">What to Expect</h2>
            </div>
          </FadeInSection>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {whatToExpect.map((item, index) => (
              <FadeInSection key={item.title} delay={index * 0.15}>
                <div className="flex items-start gap-6 p-6 gaming-card hover:border-zari-gold/20 transition-all duration-300">
                  <span className="font-cinzel text-zari-gold text-sm shrink-0 mt-1 drop-shadow-[0_0_6px_rgba(212,175,55,0.3)]">0{index + 1}</span>
                  <div>
                    <h3 className="font-playfair text-lg text-ivory mb-2">{item.title}</h3>
                    <p className="font-dm-sans text-sm text-ivory/40 leading-relaxed">{item.description}</p>
                  </div>
                </div>
              </FadeInSection>
            ))}
      ...
      <section className="section-luxury bg-noir">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
              ...
              <div className="space-y-6">
                ...
                <div className="pt-4">
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center gap-2 px-6 py-3 border border-zari-gold/40 text-zari-gold font-dm-sans text-xs tracking-wider uppercase hover:bg-zari-gold hover:text-noir hover:shadow-[0_0_20px_rgba(212,175,55,0.3)] transition-all duration-500"
                  >
                    <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
            ...
            <FadeInSection delay={0.2}>
              <div className="aspect-[4/3] bg-gradient-to-br from-noir-soft via-burgundy-deep/10 to-noir rounded-sm overflow-hidden gaming-card">
